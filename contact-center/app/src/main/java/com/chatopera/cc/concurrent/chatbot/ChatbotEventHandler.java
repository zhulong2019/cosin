...
import com.chatopera.cc.app.nlp.NlpServiceImpl;
...
public class ChatbotEventHandler implements EventHandler<UserDataEvent> {
     ...
     * @param
     */
    private void updateAgentUserWithRespData(final String userid, final String orgi, String result) {
...
//        }
        if (result == null) {
            agentUser.setChatbotlogicerror(agentUser.getChatbotlogicerror() + 1);//记录不成常的查询
        }
...
//        JSONObject result = bot.conversation(request.getUserid(), request.getMessage());
        ...
        String result = nlpService.getAnswer(request.getMessage());
        // parse response
//        logger.info("[chatbot disruptor] chat response {}", result.toString());
        logger.info("[chatbot disruptor] chat response {}", result);
//        if (result.getInt(RestUtils.RESP_KEY_RC) == 0) {
        ...
        if (result != null && !"".equals(result)) {
            ...
            resp.setMessage("哎呀，您的问题太难啦！您可以尝试问我社保、工资、考勤和放假哦！");
        }
        // reply
...
//                updateAgentUserWithRespData(request.getUserid(), request.getOrgi(), data); // 更新聊天机器人累计值
        updateAgentUserWithRespData(request.getUserid(), request.getOrgi(), result); // 更新聊天机器人累计值
